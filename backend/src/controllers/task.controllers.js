...
const submitTask = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // Files ya links - kam se kam ek to ho
        const submissionLinks = normalizeLinks(
            parseJSONField(req.body.links, "links"),
            req.user._id,
        )
            .slice(0, 10)
            .map(({ title, url }) => ({ title, url }));

        if (!uploadedFiles.length && !submissionLinks.length) {
            throw new ApiError(400, "Please add at least one file or link");
        }
        ...
        // Status "In Review" ho gaya - sabke Kanban / list update ho
        await emitToProjectMembers(projectId, "task-updated", task);
...
const reviewTaskSubmission = asyncHandler(async (req, res) => {
    ...
    // Reject karte waqt batana zaroori hai ki kya theek karna hai
    if (status === "rejected" && !feedback?.toString().trim()) {
        throw new ApiError(400, "Please add feedback so the member knows what to fix");
    }

    const submittedBy = submission.submittedBy;

    submission.status = status;
    submission.feedback = (feedback || "").toString().trim().slice(0, 2000);
...
// Status badla (Completed / In Progress) - sabke Kanban / list update ho
await emitToProjectMembers(projectId, "task-updated", task);
    ...
    message:
        status === "approved"
            ...
            : `Changes requested on "${task.title}": ${submission.feedback.slice(0, 120)}`,
    type: "task",
});
